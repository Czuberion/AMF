...
def main():
    ...
    with col1:
        ...
        min_precip, max_precip = (
            data["precipitation"].dropna().min(),
            data["precipitation"].dropna().max(),
        )
        ...
        # Restaurant_latitude
        min_lat, max_lat = (
            data["Restaurant_latitude"].dropna().min(),
            data["Restaurant_latitude"].dropna().max(),
        )
        ...
        min_delivery_lat, max_delivery_lat = (
            data["Delivery_location_latitude"].dropna().min(),
            data["Delivery_location_latitude"].dropna().max(),
        )
        delivery_location_latitude = st.slider(
            "Delivery Location Latitude",
            float(min_delivery_lat),
            float(max_delivery_lat),
            step=0.001,
        ...
        min_distance, max_distance = (
            data["Distance (km)"].dropna().min(),
            data["Distance (km)"].dropna().max(),
        )
        distance = st.slider(
            "Distance",
            float(min_distance),
    ...
    with col2:
        # Delivery_person_Age
        min_age, max_age = int(data["Delivery_person_Age"].dropna().min()), int(
            data["Delivery_person_Age"].dropna().max()
        )
        ...
        min_long, max_long = (
            data["Restaurant_longitude"].dropna().min(),
            data["Restaurant_longitude"].dropna().max(),
        )
        ...
        min_delivery_long, max_delivery_long = (
            data["Delivery_location_longitude"].dropna().min(),
            data["Delivery_location_longitude"].dropna().max(),
        )
        delivery_location_longitude = st.slider(
            "Delivery Location Longitude",
            float(min_delivery_long),
            float(max_delivery_long),
            step=0.001,
    ...
    with col3:
        ...
        type_of_order = st.radio(
            "Type of Order", options=order_types, key="type_of_order"
        )

        # Type_of_vehicle
        vehicle_types = data["Type_of_vehicle"].dropna().unique()
        type_of_vehicle = st.radio(
            "Type of Vehicle", options=vehicle_types, key="type_of_vehicle"
        )

        # Traffic_Level
        traffic_levels = ["Very Low", "Low", "Moderate", "High", "Very High"]
        traffic_level = st.radio(
            "Traffic Level", options=traffic_levels, key="traffic_level"
        )
    ...
    with col4:
        ...
        temperature = st.text_input(
            "Temperature (°C)", value="0", key="temperature")
        try:
